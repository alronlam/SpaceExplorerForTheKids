...
namespace ReachForTheStars
{
    ...
    /// </summary>
    public class ScreenSaturn : ScreenPlanet
    {
        public ScreenSaturn(Game1 game, Background background, List<Button> listButtons) 
            ...
            if (touchCollection.Count() > 0)
            {
                ...
                if (touch.State == TouchLocationState.Pressed)
                {
                    ...
                    if (command.Equals("moon"))
                    {
                        ...
                        AnimationFixed background = new AnimationFixed(game.textureSaturnMoonInfo, 1, 0, 800, 480, 0, 0, 0);
                        NextScreen = new ScreenInfo(game, new Background(background, null), null, this);
                    }
                    else if (command.Equals("ring"))
                    {

                        AnimationFixed background = new AnimationFixed(game.textureSaturnRingInfo, 1, 0, 800, 480, 0, 0, 0);
                        NextScreen = new ScreenInfo(game, new Background(background, null), null, this);
                    }
                    else if (command.Equals("clock"))
                    {

                        AnimationFixed background = new AnimationFixed(game.textureSaturnClockInfo, 1, 0, 800, 480, 0, 0, 0);
                        NextScreen = new ScreenInfo(game, new Background(background, null), null, this);
                    }
                }
            }
